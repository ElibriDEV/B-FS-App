...
export class JwtStrategy extends PassportStrategy(Strategy, 'jwt-exp-ignore') {
  ...
  private static extractJWT(req: Request): string | null {
    if (req.cookies && 'access_token' in req.cookies) return req.cookies['access_token'];
    return null;
  }

  async validate(payload: AccessPayloadInterface | null): Promise<AccessPayloadInterface> {
